...
for i in range(0, 16):
    for j in range (0, 10):
        if (i + j) % 2 == 0:
            box(length=dimTile, width=dimTile, pos = vector (-80 + i * dimTile, posY, -50 + j * dimTile ), color=white, material=rough)
        else:
            box(length=dimTile, width=dimTile, pos = vector (-80 + i * dimTile, posY, -50 + j * dimTile ), color=color.gray(.4), material=marble)

#Beer Bottle
beerBottle = frame()
ring (frame = beerBottle, axis = up, radius = 1, thickness = 0.1, color = vector(1, .65, 0), material = plastic, opacity = .6, pos = vector(0, 2, 0) )
cylinder (frame = beerBottle, axis = up, radius = 1.1, color = vector(1, .65, 0), material = plastic, opacity = .6, pos = vector(0, 2, 0), length = 3)
cone(frame = beerBottle, pos = (0, 5, 0), axis = (0, 2, 0), radius = 1.1, color = vector(1, .65, 0), material = plastic, opacity = .6)
cylinder (frame = beerBottle, axis = up, radius = .5, color = vector(1, .65, 0), material = plastic, opacity = .6, pos = vector(0, 5.25, 0), length = 2)
ring (frame = beerBottle, axis = up, radius = .5, thickness = 0.05, color = color.gray(.6), material = plastic, pos = vector(0, 7.25, 0) )
cylinder (frame = beerBottle, axis = up, radius = .55, color = color.gray(.6), material = plastic, pos = vector(0, 7.25, 0), length = .1)
beerBottle.pos += (-40, 0, -28)
...
#Chalk
box ( color = white, material = rough, pos = vector(-35, 2.5, -28) )
box ( color = color.blue, pos = vector(-35, 3.1, -28), height = .2)
ring (axis = up, radius = .4, thickness = .1, color = color.blue, pos = vector(-35, 3.2, -28) )
#Cylinder Chalk
cylinder ( color = white, material = rough, length = 1, radius = .5, pos = vector(-31, 2, -28), axis = up )
cylinder ( color = color.blue, radius = .5, length = .25, pos = vector(-31, 3, -28), height = .2, axis = up )
ring (axis = up, radius = .4, thickness = .1, color = color.blue, pos = vector(-31, 3.25, -28) )
